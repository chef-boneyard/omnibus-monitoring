...
Vagrant::Config.run do |config|
  ...
  config.vm.share_folder "omnibus-monitoring", "~/omnibus-monitoring", File.expand_path("..", __FILE__)
  ...
  config.vm.provision :chef_solo do |chef|
    ...
    chef.json = {
      "omnibus" => {
        "install-dirs" => ["/opt/opscode-monitoring"]
      }
    }
  end
  ...
  config.omnibus.path = "~/omnibus-monitoring"
end
